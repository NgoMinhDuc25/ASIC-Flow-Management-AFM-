...
class AFMApp(tk.Tk):
    ...
    # ------------------------------------------------------------------ #
    # Window icon / avatar (title bar + taskbar)
    # ------------------------------------------------------------------ #
    def _set_window_icon(self) -> None:
        """
        Set the title-bar / taskbar icon (what the user calls the "avatar").
        `self.title(...)` only sets the text — the icon is a separate call.

        Loads several PNG sizes so the OS picks a crisp one for title bar,
        taskbar, and Alt-Tab switcher. Falls back to icon.ico on Windows,
        and fails silently if no icon asset is bundled (title-only window).
        """
        try:
            ...
            if photos:
                # keep a reference so PhotoImage objects aren't garbage-collected
                self._icon_images = photos
            ...
            # Never let a missing/broken icon asset crash the app.
            pass
